...
namespace Moban.DAL
{
    public class MobanDAL
    {
        ...
        public bool Exists(string Url)
        {
            ...
            strSql.Append("select count(1) from moban");
        ...
        public bool PartsExists(string Url)
        {
            StringBuilder strSql = new StringBuilder();
        ...
        /// 增加一条数据
        /// </summary>
        public int Add(Model.MobanInfo model)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("insert into moban(");
            strSql.Append("[Name],[url],[Host],[mpic],[bpic],[body],[Demo],[Type],[CreateTime],[CrawlTime])");
            ...
            parameters[9].Value = model.CrawlTime;
        ...
        {
            string strSql = "select top 1000 id,body from moban where IsAnalyze is null order by id asc";
            return DbHelperSQL.Query(strSql.ToString());
        ...
        {
            string strSql = string.Format("select  top 1 * from moban  where demourl like '%{0}%'", filename);
        ...
        public bool UpdateIsDownLoad(Model.MobanInfo model)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("Update moban set IsDownLoad=@IsDownLoad where Id=@Id;");
            SqlParameter[] parameters = {
